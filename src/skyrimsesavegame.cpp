...
SkyrimSESaveGame::SkyrimSESaveGame(QString const& fileName, GameSkyrimSE const* game) : GamebryoSaveGame(fileName, game, true)
{
  FileWrapper file(fileName, "TESV_SAVEGAME");  // 10 bytes
  ...
  // For some reason, the file time is off by about 6 hours. So we need to subtract those 6 hours from the filetime.
...
void SkyrimSESaveGame::fetchInformationFields(FileWrapper& file, unsigned long& version, QString& playerName, unsigned short& playerLevel, QString& playerLocation, unsigned long& saveNumber, FILETIME& creationTime) const
{
...
{
  FileWrapper file(getFilepath(), "TESV_SAVEGAME");  // 10 bytes
  ...
  unsigned long version = 0;
  {
    ...
    fetchInformationFields(file, version, dummyName, dummyLevel, dummyLocation, dummySaveNumber, dummyTime);
  }
